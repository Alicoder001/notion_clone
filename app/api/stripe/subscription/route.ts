...
export async function POST(req: Request) {
  try {
    ...
    const isExitingCustomer = await stripe.customers.list({ email });
    let customer;
    if (isExitingCustomer.data.length) {
      customer = isExitingCustomer.data[0];
    }
    if (!customer) {
    ...
    return NextResponse.json(
      "Something went wrong. Place try again!  -" + "Error: " + error,
      {
        status: 500,
      }
    );
...
export async function GET(req: Request) {
  try {
    ...
    return NextResponse.json("Something went wrong. Place try again!", {
      status: 500,
    });
  }
}
